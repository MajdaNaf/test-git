...
// for trace purposes - display all rules in text format
 ...
 public void displayConflictSet(Vector ruleSet) {
   textArea1.appendText("\n" + " -- Rgèles dans la liste de conflits :\n");
 ...
 public void reset() {
   textArea1.appendText("\n --- Initialisation des variables de la " + name + " à null");
   Enumeration enume = variableList.elements() ;
   while(enume.hasMoreElements()) {
     RuleVariable temp = (RuleVariable)enume.nextElement() ;
     temp.setValue(null) ;
...
//     if rule is null then we couldn't find a value
//
...
while (goalClauses.hasMoreElements()) {
   ...
   if (ruleTruth == null) {
      textArea1.appendText("\nRule " + goalRule.name +
                        " est null, impossible d'évaluer sa valeur de vérité.");
     ...
     if (goalClauseStack.empty() == true) {
       textArea1.appendText("\n +++ Solutions trouvées pour le but : "
                            + goalVar.name);
       break ; // for now, only find first solution, then stop
     }
   } else {
     goalClauseStack.pop() ; // clear item from subgoal stack
     textArea1.appendText("\nLa règle " + goalRule.name +
                          " est fausse, impossible d'assigner la variable " + goalVar.name);
   }
 }

 if (goalVar.value == null) {
   textArea1.appendText("\n +++ Impossible de trouver une solution pour ce but : "
                         + goalVar.name);
 }
}
